...
description: >
  36 behavioral rules that cut AI output tokens ~59% while maintaining quality.
  Always-on rules: no filler, no echo, act first, proportional responses, diffs over rewrites,
  cheapest tool first, parallelize calls, compact early. Drop-in for any project.
  Based on benchmark results across 15 tasks, 7 categories, 3-run average.
---
...
## Images
32. Resize before reading. macOS: `sips --resampleHeightWidthMax 1200 {path} --out /tmp/{name}`. Linux: `convert {path} -resize 1200x1200\> /tmp/{name}`.
